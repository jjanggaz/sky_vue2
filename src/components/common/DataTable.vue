<template>
  <div
    ...
    :class="{ 'with-scroll': maxHeightValue !== 'auto' }"
    :style="maxHeightValue !== 'auto' ? { maxHeight: maxHeightValue } : {}"
  >
    <table class="data-table">
      <thead>
        <tr>
          <th v-if="props.selectable" class="checkbox-cell">
            <template v-if="showSelectAllValue && selectionModeValue !== 'none'">
              <input
                type="checkbox"
                :checked="allSelected"
                :disabled="data.length === 0 || selectionModeValue === 'single'"
      ...
      <tbody>
        <tr v-if="loadingValue" class="loading-row">
          <td
            :colspan="getColspan()"
        ...
        <template v-else>
          ...
          >
            <td v-if="props.selectable" class="checkbox-cell" @click.stop>
              <input
                type="checkbox"
...
const rowKeyValue = computed(() => props.rowKey ?? "id");
const selectedItemsValue = computed(() => props.selectedItems ?? []);
const selectionModeValue = computed(() => props.selectionMode ?? "multiple");
const showSelectAllValue = computed(() => props.showSelectAll ?? true);
const maxHeightValue = computed(() => props.maxHeight ?? "auto");
...
// 선택 상태 관리
const localSelected = ref<any[]>([...selectedItemsValue.value]);
...
const getRowClass = (item: any, index: number) => {
  const globalDraggedIndex = (window as any).__draggedRowIndex;
  const isDraggingRow = isDragging.value && (draggedIndex.value === index || globalDraggedIndex === index);
  return [
    'data-row',
    {
      selected: isSelected(item),
      'drag-over': dragOverIndex.value === index,
      dragging: isDraggingRow
    }
  ];
};
...
const toggleSelectRow = (item: any) => {
  ...
  if (index > -1) {
    ...
    // 선택되지 않은 경우 선택
    if (selectionModeValue.value === "single") {
...
const handleRowClick = (item: any, index: number) => {
  ...
  if (props.selectable && selectionModeValue.value !== "none") {
    if (selectionModeValue.value === "single") {
...
const getRowKey = (item: any, index: number) => {
  return item[rowKeyValue.value] || index;
};
